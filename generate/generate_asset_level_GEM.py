...
NOTES:
    - NB. The code is currently quite repetitive as every sector is processed separately. This could be handled by more
            generic functions.
"""
# import packages
import os
...
def process_and_save_gem_data():
    """
    Collect and prepare asset-level data from the Global Energy Monitor (GEM) database.

    This function processes data from various asset-level datasets (e.g., wind, steel, solar, oil and gas extraction,
    nuclear, hydropower, geothermal, coal terminals, coal plants, coal mines, LNG terminals, bioenergy) provided by
    the Global Energy Monitor (GEM). The steps include:
    ...
    Returns:
        pd.DataFrame: A consolidated and cleaned DataFrame containing asset-level data from GEM.
    """
    ...
    df_steel["capacity"] = df_steel["capacity"] / 1000
    ...
    df_steel["parent_[formula]"] = df_steel["parent_[formula]"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                            regex=True).str.strip()
    df_steel["parent_permid_[formula]"] = df_steel["parent_permid_[formula]"].str.replace(
        "[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '', regex=True).str.strip()
    ...
    df_solar["capacity_unit"] = "mw"  # mw (peak value, grid connected, or unknown)
    ...
    df_solar["operator"] = df_solar["operator"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                            regex=True).str.strip()
    ...
    df_oil_gas_extraction["parent"] = df_oil_gas_extraction["parent"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                                  regex=True).str.strip()

    df_oil_gas_extraction["owner"] = df_oil_gas_extraction["owner"].str.split(";", expand=True)[0]
    df_oil_gas_extraction["owner"] = df_oil_gas_extraction["owner"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                                regex=True).str.strip()

    df_oil_gas_extraction["operator"] = df_oil_gas_extraction["operator"].str.split(";", expand=True)[0]
    df_oil_gas_extraction["operator"] = df_oil_gas_extraction["operator"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]",
                                                                                      '', regex=True).str.strip()
    ...
    df_nuclear["operator"] = df_nuclear["operator"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                regex=True).str.strip()
    ...
    df_hydropower["owner"] = df_hydropower["owner"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                regex=True).str.strip()

    df_hydropower["operator"] = df_hydropower["operator"].str.split(";", expand=True)[0]
    df_hydropower["operator"] = df_hydropower["operator"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                      regex=True).str.strip()
    ...
    df_geothermal["owner"] = df_geothermal["owner"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                regex=True).str.strip()

    df_geothermal["operator"] = df_geothermal["operator"].str.split(";", expand=True)[0]
    df_geothermal["operator"] = df_geothermal["operator"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                      regex=True).str.strip()
    ...
    df_coal_terminals["capacity_unit"] = "mt per year"

    # CLEAN OWNER
    df_coal_terminals["owner"] = df_coal_terminals["owner"].str.split(";", expand=True)[0]
    df_coal_terminals["owner"] = df_coal_terminals["owner"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                        regex=True).str.strip()
    ...
    df_coal_plant["owner"] = df_coal_plant["owner"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                regex=True).str.strip()

    df_coal_plant["parent"] = df_coal_plant["parent"].str.split(";", expand=True)[0]
    df_coal_plant["parent"] = df_coal_plant["parent"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                  regex=True).str.strip()
    ...
    df_coal_mine["owners"] = df_coal_mine["owners"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                regex=True).str.strip()

    df_coal_mine["parent_company"] = df_coal_mine["parent_company"].str.split(";", expand=True)[0]
    df_coal_mine["parent_company"] = df_coal_mine["parent_company"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                                regex=True).str.strip()
    ...
    df_lng_terminals["owner"] = df_lng_terminals["owner"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                      regex=True).str.strip()

    df_lng_terminals["parent"] = df_lng_terminals["parent"].str.split(";", expand=True)[0]
    df_lng_terminals["parent"] = df_lng_terminals["parent"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                        regex=True).str.strip()
    ...
    df_bioenergy["owner"] = df_bioenergy["owner"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                              regex=True).str.strip()

    df_bioenergy["operator"] = df_bioenergy["operator"].str.split(";", expand=True)[0]
    df_bioenergy["operator"] = df_bioenergy["operator"].str.replace("[\[\(]\d*[.]?\d*[%][\s*]?[\]\)]", '',
                                                                    regex=True).str.strip()
